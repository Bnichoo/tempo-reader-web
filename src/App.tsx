/** Tempo Reader – App.tsx (guarded, workerized) */
...
function uuid() {
  try { if (globalThis.crypto && 'randomUUID' in globalThis.crypto) return (globalThis.crypto as any).randomUUID() } catch {}
...
function sentenceRangeAt(tokens: string[], i: number): RangeT {
  ...
  let s = 0; for (let k = i - 1; k >= 0; k--) { if (isBoundary(tokens[k])) { s = k + 1; break } }
  while (s < tokens.length && /^\s+$/.test(tokens[s])) s++
  let e = tokens.length - 1; for (let k = i; k < tokens.length; k++) { if (isBoundary(tokens[k])) { e = k; break } }
...
type SettingsV1 = {
  wps: number; count: number; gap: number;
  focusScale: number; dimScale: number; dimBlur: number;
  fontPx: number; dark: boolean; drawerOpen: boolean;
}
...
function loadSettings(): Partial<SettingsV1> | null { try { return JSON.parse(localStorage.getItem(K_SETTINGS) || "null"); } catch { return null } }
function saveSettings(s: SettingsV1) { try { localStorage.setItem(K_SETTINGS, JSON.stringify(s)) } catch {} }
function loadClips(): Clip[] { try { const a = JSON.parse(localStorage.getItem(K_CLIPS) || "[]") as Clip[]; return Array.isArray(a)?a:[] } catch { return [] } }
function saveClips(list: Clip[]) { try { localStorage.setItem(K_CLIPS, JSON.stringify(list)) } catch {} }

function exportDataFile(payload: { settings: SettingsV1; clips: Clip[] }) {
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" })
  const url = URL.createObjectURL(blob); const a = document.createElement("a")
  a.href = url; a.download = `tempo-reader-export-${new Date().toISOString().slice(0,10)}.json`
  document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url)
}
function parseImport(json: string): { settings?: Partial<SettingsV1>; clips?: Clip[] } {
...
function clampSettings(s: Partial<SettingsV1>): Partial<SettingsV1> {
  ...
  if (r.wps != null)        r.wps = clamp(Number(r.wps), 0.5, 3)
  if (r.count != null)      r.count = clamp(Number(r.count), 1, 7)
  if (r.gap != null)        r.gap = clamp(Number(r.gap), 0.20, 0.80)
  if (r.focusScale != null) r.focusScale = clamp(Number(r.focusScale), 1.00, 1.60)
  if (r.dimScale != null)   r.dimScale = clamp(Number(r.dimScale), 0.85, 1.00)
  if (r.dimBlur != null)    r.dimBlur = clamp(Number(r.dimBlur), 0, 2.5)
  if (r.fontPx != null)     r.fontPx = clamp(Number(r.fontPx), 16, 28)
  return r
}
...
function pruneClips(list: Clip[]): Clip[] {
  ...
  const rest = list.filter(c => !c.pinned).sort((a,b)=> (new Date(b.createdUtc).getTime()) - (new Date(a.createdUtc).getTime()))
...
function NoteEditorModal(props: {
  open: boolean; draftKey?: string; initialHtml?: string;
  ...
  const [active, setActive] = useState<{ b: boolean; i: boolean; u: boolean; link: boolean }>({
    b: false, i: false, u: false, link: false
  })
  ...
  function getAnchorInSelection(): HTMLAnchorElement | null {
    ...
    let node: Node | null = sel.anchorNode
    while (node && node !== root) { if (node instanceof HTMLAnchorElement) return node; node = node.parentNode }
    return null
  }
  ...
  function exec(cmd: string, value?: string) {
    ...
    try { if (typeof document.execCommand === 'function') document.execCommand(cmd, false, value) } catch (e) { console.warn('execCommand failed', cmd, e) }
  ...
  function createOrEditLink() {
    ...
    if (!href) { alert("Invalid URL. Use https://, http://, mailto:, or tel:."); return }

    const sel = window.getSelection()
    if (sel && sel.rangeCount > 0 && sel.getRangeAt(0).collapsed) {
      const zwsp = document.createTextNode("\u200B")
      const r = sel.getRangeAt(0); r.insertNode(zwsp)
      sel.removeAllRanges(); const nr = document.createRange(); nr.setStartBefore(zwsp); nr.setEndAfter(zwsp); sel.addRange(nr)
    }
    try { if (typeof document.execCommand === 'function') document.execCommand('createLink', false, href) } catch (e) { console.warn('createLink failed', e) }
    if (editorRef.current) fixupAllAnchors(editorRef.current); updateToolbarStates(); pushHistorySnapshot(true)
  }
  function unlinkSelection() {
    const ed = editorRef.current; if (!ed) return
    ed.focus(); restoreSelection()
    try { if (typeof document.execCommand === 'function') document.execCommand('unlink') } catch (e) { console.warn('unlink failed', e) }
  ...
  function insertSanitizedHtml(html: string) {
    ...
    if (ok) { try { document.execCommand('insertHTML', false, safe) } catch (e) { console.warn('insertHTML failed; falling back', e) } }
    else {
...
export default function App() {
  ...
  useEffect(() => {
    const onBIP = (e: any) => { e.preventDefault(); deferredPromptRef.current = e; setCanInstall(true) }
  ...
  const wordIndexFromToken = (ti:number)=>{ const a=wordIdxData.starts; if(!a.length) return 0; let lo=0,hi=a.length-1,ans=0; while(lo<=hi){const m=(lo+hi)>>1; if(a[m]<=ti){ans=m;lo=m+1;} else {hi=m-1;}} return ans }
  ...
  const [readerRatio,setReaderRatio] = useState(0.70)
  ...
  useEffect(() => {
    function onMove(ev: MouseEvent) {
      ...
      const ratio = clamp(y / rect.height, 0.30, 0.85)
  ...
  useEffect(() => {
    ...
    const c = { ...s }
    const sc = (p: any, k: string, f: (v:any)=>void) => { if (p[k] != null) f(p[k]) }
    sc(c,"wps",(v)=>setWps(clamp(Number(v),0.5,3)))
    sc(c,"count",(v)=>setCount(clamp(Number(v),1,7)))
    sc(c,"gap",(v)=>setGap(clamp(Number(v),0.20,0.80)))
    sc(c,"focusScale",(v)=>setFocusScale(clamp(Number(v),1.00,1.60)))
    sc(c,"dimScale",(v)=>setDimScale(clamp(Number(v),0.85,1.00)))
  ...
  useEffect(() => {
    backupRef.current = { settings: { wps, count, gap, focusScale, dimScale, dimBlur, fontPx, dark, drawerOpen }, clips }
  }, [wps, count, gap, focusScale, dimScale, dimBlur, fontPx, dark, drawerOpen, clips])
  useEffect(() => {
    const doBackup = () => {
      try {
        const data = backupRef.current; if (!data) return
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      const dt = (now - last) / 1000; last = now; acc += dt * wps
      if (acc >= 1) {
  ...
  const onFile = useCallback(async (file: File) => {
    if (file.size > 4 * 1024 * 1024) {
      alert("This file is quite large (>4MB). It may feel slow in the browser.")
    }
    const ext = file.name.toLowerCase().split(".").pop()
    const raw = await file.text(); let clean = raw
    if (ext === "html" || ext === "htm") {
      const parser = new DOMParser(); const doc = parser.parseFromString(raw, "text/html"); clean = doc.body.innerText
    }
    setText(clean)
  }, [])

  const focusTokenStart = useMemo(() => tokenIndexFromWord(wIndex), [wIndex, wordIdxData])
  const focusTokenEnd   = useMemo(() => tokenEndFromWord(Math.min(wIndex + count - 1, wordIdxData.count - 1)), [wIndex, count, wordIdxData])
  const focusRange      = useMemo(() => ({ start: focusTokenStart, length: Math.max(1, focusTokenEnd - focusTokenStart + 1) }), [focusTokenStart, focusTokenEnd])
  ...
  const handleSaveNote = useCallback((html: string) => {
    ...
    if (editingClipId) {
      setClips(prev => pruneClips(prev.map(c => c.id === editingClipId ? { ...c, noteHtml: empty ? undefined : safe } : c)))
      setEditingClipId(null); setNoteOpen(false); setHoverRange(null); if (!aidAuto) setAidRange(null)
      return
    }
    ...
    const newClip: Clip = {
      id: uuid(),
      start, length, snippet,
      noteHtml: empty ? undefined : safe,
      createdUtc: new Date().toISOString(),
      pinned: false
    }
    setClips(prev => pruneClips([newClip, ...prev]))
    setPendingRange(null); setNoteOpen(false); setHoverRange(null); if (!aidAuto) setAidRange(null)
  }, [editingClipId, pendingRange, tokens, aidAuto])

  const togglePin = (id: string) => setClips(prev => prev.map(c => c.id === id ? { ...c, pinned: !c.pinned } : c))
  const moveClip  = (id: string, dir: -1 | 1) => setClips(prev => {
    const ix = prev.findIndex(c => c.id === id); if (ix < 0) return prev
    const jx = ix + dir; if (jx < 0 || jx >= prev.length) return prev
    const next = prev.slice(); const [item] = next.splice(ix, 1); next.splice(jx, 0, item); return next
  })
  const deleteClip = (id: string) => { setClips(prev => prev.filter(c => c.id !== id)); setHoverRange(null); if (!aidAuto) setAidRange(null) }

  const [editRangeForId,setEditRangeForId] = useState<string|null>(null)
  const beginEditRange = (id: string) => { setEditRangeForId(id); setCurrentSelection(null) }
  const applyEditedRange = () => {
    ...
    const clip = clips.find(c => c.id === editRangeForId)
    if (clip) setWIndex(wordIndexFromToken(clip.start))
    setEditRangeForId(null); setHoverRange(null)
  }
  ...
  const filteredClips = useMemo(() => {
    ...
    const list = q
      ? clips.filter(c => c.snippet.toLowerCase().includes(q) || (c.noteHtml && stripHtml(c.noteHtml).toLowerCase().includes(q)))
      : clips.slice()
  ...
  const computedGapEm = Math.max(gap, 0.20 + guardExtra)
  ...
  useEffect(() => {
    ...
    events.forEach(ev => el.addEventListener(ev, handler as any, { passive: true } as any))
    return () => {
      if (timer) window.clearTimeout(timer)
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === "Space") { e.preventDefault(); setPlaying(p => !p) }
        ...
        const range = currentSelection ? currentSelection : (aidAuto ? sentenceRangeAt(tokens, tokenStart) : focusRange)
        setPendingRange(range); setEditingClipId(null); setNoteOpen(true)
      } else if (e.key === "Escape") { setAidRange(null) }
    }
    ...
    <>
      <header className="sticky top-0 z-20 backdrop-blur bg-sepia-50/80 border-b border-sepia-200">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <label className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-sepia-200 bg-white/70 hover:bg-white transition cursor-pointer ml-2">
            <Upload /><span className="text-sm">Open .txt / .html</span>
            <input type="file" accept=".txt,.html,.htm,.md" className="hidden"
      ...
      >
        ...
        <div className="my-3">
          ...
          <input type="range" min={1} max={7} step={1} value={count} onChange={(e)=>setCount(clamp(parseInt(e.target.value,10),1,7))}/>
        </div>

        <div className="my-3">
          <div className="text-xs text-sepia-700 mb-1">↔️ Text spacing: {gap.toFixed(2)}em</div>
          <input type="range" min={0.20} max={0.80} step={0.01} value={gap} onChange={(e)=>setGap(parseFloat(e.target.value))}/>
        </div>

        <div className="my-3">
          <div className="text-xs text-sepia-700 mb-1">🔎 In-focus size: {focusScale.toFixed(2)}×</div>
          <input type="range" min={1.00} max={1.60} step={0.01} value={focusScale} onChange={(e)=>setFocusScale(parseFloat(e.target.value))}/>
        </div>

        <div className="my-3">
          <div className="text-xs text-sepia-700 mb-1">🧘 Out-of-focus size: {dimScale.toFixed(2)}×</div>
          <input type="range" min={0.85} max={1.00} step={0.01} value={dimScale} onChange={(e)=>setDimScale(parseFloat(e.target.value))}/>
        </div>
        ...
        <div className="my-4 border-t border-sepia-200 pt-3">
          ...
          <label className="inline-flex items-center gap-2 px-3 py-2 rounded-xl border border-sepia-200 bg-white/70 hover:bg-white transition cursor-pointer">
            ⤴ Import Settings
            <input type="file" accept="application/json" className="hidden" onChange={(e)=> onImportJson(e.target.files?.[0] ?? null)} />
          </label>
          <button className="mt-2 inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-sepia-200 bg-white/70 hover:bg-white transition"
            onClick={doExport} title="Export clips + settings">
            ⤵ Export Settings
          </button>
        ...
        <div className="my-3 flex items-center gap-2">
          <input id="aidAuto" type="checkbox" checked={aidAuto} onChange={(e)=>{ setAidAuto(e.target.checked); if(!e.target.checked) setAidRange(null) }} />
          <label htmlFor="aidAuto" className="text-sm">Sentence highlight (auto)</label>
        </div>
      ...
      <div className="page-wrap" style={{ ["--drawer-offset" as any]: `${drawerOffset}px` }}>
        <main className="max-w-5xl mx-auto p-4 h-[calc(100vh-64px)] md:h-[calc(100vh-64px)]">
          <div ref={containerRef} className="w-full h-full flex flex-col min-h-0">
            ...
            <div className="resizer" onMouseDown={(e)=>{e.preventDefault(); (draggingRef as any).current=true}} title="Drag to resize" />

            <section className="rounded-2xl bg-white border border-sepia-200 overflow-hidden" style={{ height:`calc(${((1-readerRatio)*100).toFixed(2)}% - 4px)` }}>
              <div className="px-4 py-3 border-b border-sepia-200 flex items-center justify-between gap-3">
                <div className="font-medium">Clips <span className="text-sepia-700">({clips.length})</span></div>
                <input className="px-3 py-1.5 rounded-xl border border-sepia-200 bg-white/70 w-72" placeholder="Search clips…" value={query} onChange={(e)=>setQuery(e.target.value)}/>
              </div>
              ...
              {editRangeForId && (
                <div className="px-4 py-2 bg-sepia-50 border-b border-sepia-200 flex items-center gap-3 text-sm">
                  ...
                  <button
                    className={"px-3 py-1 rounded-lg border " + (currentSelection ? "border-sepia-400" : "border-sepia-200 opacity-50")}
                    disabled={!currentSelection}
                    onClick={applyEditedRange}
                  >
                    Use current selection
                  </button>
                  <button className="px-3 py-1 rounded-lg border border-sepia-200" onClick={()=> setEditRangeForId(null)}>Cancel</button>
                </div>
              )}
                    ...
                    {(query ? filteredClips : clips).map(c => (
                      ...
                      >
                        ...
                        <div className="text-sm">
                          ...
                          {c.pinned && <span className="ml-2 text-xs px-2 py-0.5 rounded-full border border-sepia-200">Pinned</span>}
                        </div>
                        ...
                        <div className="text-xs text-sepia-700 mt-1">{new Date(c.createdUtc).toLocaleString()}</div>
                      </li>
                    ))}
                  </ul>
                )}
        ...
        <NoteEditorModal
          open={noteOpen}
          draftKey={editingClipId ? `draft:clip:${editingClipId}` : (pendingRange ? `draft:new` : undefined)}
          initialHtml={editingClipId ? (clips.find(c => c.id === editingClipId)?.noteHtml || "") : ""}
          onCancel={()=>{ setNoteOpen(false); setEditingClipId(null); setPendingRange(null) }}
